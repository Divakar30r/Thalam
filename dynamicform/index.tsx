...
import { INDUSTRY_DATA as MOCK_INDUSTRY_DATA } from './mock-data.tsx';
...
// --- API CONFIG ---
import { API_BASE_URL, PRESIGNED_URL_CONFIG_NAME, PRESIGNED_URL_ENDPOINT } from './config';
...
/*
const ProposalComparisonFrame = ({ industry, product: originalProduct, onSave, isRejected }) => {
  const [formData, setFormData] = useState<any>({});

  useEffect(() => {
    const initialFactors = {};
    Object.entries(industry.factorskey || {}).forEach(([key, valueDef]) => {
      initialFactors[key] = getFactorAsString(key, valueDef, originalProduct.factors?.[key]);
    });
    setFormData({
      ...originalProduct,
  ...
  }, [originalProduct, industry]);
    
  const sortedFactors = useMemo(() => {
    if (!industry?.factorskey || !originalProduct?.factors) return [];
    return Object.entries(industry.factorskey).sort(([keyA], [keyB]) => {
      const valueA = originalProduct.factors?.[keyA];
      const valueB = originalProduct.factors?.[keyB];
      const hasValueA = formatFactorValue(valueA) !== 'N/A';
      const hasValueB = formatFactorValue(valueB) !== 'N/A';

      if (hasValueA && !hasValueB) return -1;
      if (!hasValueA && hasValueB) return 1;
      return getLabel(keyA).localeCompare(getLabel(keyB));
    });
  }, [industry, originalProduct]);
  ...
  const handleFactorChange = (factorKey, value) => {
    setFormData(prev => ({
      ...prev,
      factors: { ...prev.factors, [factorKey]: value },
    }));
  };
...
// const ProposalPage = ({ industry, product, onClose }) => {
...
//     }
// 
//     if (!isRejected && data.needsPacking) {
//       setIsPackingVisible(true);
//     } else {
...
//       <div className="bg-gray-200 rounded-lg shadow-2xl w-full h-full max-w-screen-2xl flex flex-col">
//         <header className="p-4 flex justify-between items-center border-b bg-white rounded-t-lg flex-shrink-0 z-20">
//           <h2 className="text-2xl font-bold text-gray-800 truncate pr-4">Proposal for: {product.productName}</h2>
//           <div className="flex items-center space-x-4 flex-shrink-0">
//             <button
//               type="submit"
//               form="proposal-form"
//               className="py-2 px-4 bg-indigo-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-indigo-700"
//             >
//               Save Proposal
//             </button>
//             <button
...
//               className={`py-2 px-4 rounded-md text-sm font-medium transition-colors ${isRejected ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}
//             >
//               {isRejected ? 'Cancel Reject' : 'Reject'}
//             </button>
//             <button onClick={onClose} className="py-2 px-4 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Close</button>
//           </div>
//         </header>
//         <main className="flex-grow p-4 flex gap-4 overflow-hidden">
//           {/* Left/Main Pane */}
//           <div className={`bg-white rounded-lg shadow-sm h-full overflow-y-auto ${isPackingVisible ? 'w-[70%]' : 'w-full'} transition-all duration-300`}>
//             <ProposalComparisonFrame 
//               industry={industry} 
//               product={product} 
...
//           {isPackingVisible && (
//             <div className="w-[30%] bg-white rounded-lg shadow-sm h-full overflow-y-auto flex flex-col">
//               <h3 className="text-lg font-semibold p-4 border-b flex-shrink-0">Packing Details</h3>
...
const App = () => {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      {/* Auth check: show loading, login redirect, or welcome screen */}
      ...
      {isAuthenticated === false && (
        <div className="text-center py-20">
          <p className="text-lg text-gray-600 mb-4">You are not authenticated.</p>
          <button 
            onClick={() => window.location.href = `/auth/start?returnTo=${encodeURIComponent(window.location.origin)}`}
      ...
      <header className="mb-8">
        <h1 className="text-4xl font-bold text-gray-800">Order entry page</h1>
      </header>
          ...
          <label className="inline-flex items-center space-x-2">
            <input id="fileInput" type="file" accept="application/pdf,image/jpeg" style={{ display: 'none' }} onChange={async (e) => {
              ...
              try {
                ...
                // Success — you may want to call backend to record the document using presignData.document_id or s3_key
