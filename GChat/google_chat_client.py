...
if __name__ == "__main__":
    ...
    try:
        ...
        # Test listing spaces
        print("Testing Google Chat API client...")
        spaces = client.list_spaces()
        print(f"Found {len(spaces.get('spaces', []))} spaces")
        
        # If spaces exist, test sending a message
        if spaces.get('spaces'):
            space_name = spaces['spaces'][0]['name']
            print(f"Testing message creation in space: {space_name}")
            
            message = client.create_message(
                space_name, 
                f"Hello from {GOOGLE_CHAT_CONFIG['user_email']} via Keycloak WIF! 🎉"
            )
            print("Message sent successfully!")
            
    except Exception as e:
        print(f"Error testing Google Chat client: {e}")